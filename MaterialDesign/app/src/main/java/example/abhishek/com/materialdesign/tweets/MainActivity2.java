package example.abhishek.com.materialdesign.tweets;
...
public class MainActivity2 extends AppCompatActivity {
	...
	String[] imagetweets  ;
	String[] nametweets ;
	String[] subnametweets  ;


	int tweetcount = 0 ;
	private ListActivity activity;
	final static String SearchTerm = "swachhbharatabhiyaan";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this);
		 lv= (ListView)findViewById(R.id.lvTesti);
	...
	private class DownloadTwitterTask extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String result) {
			...
			textweets = new String[tweetcount] ;
			imagetweets = new String[tweetcount] ;
			nametweets = new String[tweetcount] ;
			subnametweets = new String[tweetcount] ;
			//int i=0 ;
			...
			for(int i=0;i<searches.size();i++){
				textweets[i]=searches.get(i).Text;
				imagetweets[i]=searches.get(i).getUser().getProfileImageUrl() ;
				nametweets[i]=searches.get(i).getUser().getName() ;
				subnametweets[i]=searches.get(i).getUser().getScreenName() ;

				//Toast.makeText(getApplicationContext(),imagetweets[i],Toast.LENGTH_SHORT).show();
			}

			TestimonialsAdapter adapter = new TestimonialsAdapter(getBaseContext(),textweets,imagetweets,nametweets,subnametweets);
			lv.setAdapter(adapter);
			//ArrayAdapter<Search> adapter = new ArrayAdapter<Search>(activity, R.layout.testi_item, searches);
